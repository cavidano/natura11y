...
Focus moves into the flyout menu content when it opens. The *Escape* key closes the menu and returns focus to the trigger element.
...
Sub-panels follow the same `.flyout__panel` pattern. The `.flyout__panel-title` wraps a `<p>` to label the current sub-panel context for the user. JavaScript automatically applies `inert` to inactive panels, hiding them from the tab order.
